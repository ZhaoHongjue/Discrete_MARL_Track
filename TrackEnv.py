...
class TrackEnv:
    ...
    def AddAgents(self, num):
        ...
        for _ in range(num):
            ...
            # 将机器人目标点设为猎物坐标
            agent = Agent(initpos = initpose, goal = self.prey.pos)
    ...
    def AddPrey(self):
        ...
        self.prey.pos[0] = goal[0]
        self.prey.pos[1] = goal[1]
    ...
    def Agents_Place_Refresh(self):
        ...
        self.map[self.prey.pos[0], self.prey.pos[1]] = 4
    ...
    def PreyMove(self):
        ...
        更新猎物位置
        '''
        temp_map = np.ones((self.size + 2, self.size + 2))
        temp_map[1:self.size + 1, 1:self.size + 1] = self.map
        temp_prey = [self.prey.pos[0] + 1, self.prey.pos[1] + 1]
        ...
        partial_map = (temp_map[u:d+1, l:r+1].reshape(-1)).tolist()
        ...
        if len(direct_idx) >= 1:
            direct = random.sample(direct_idx, 1)[0]
            if direct == 0:
                self.prey.pos[0] -= 1
                self.prey.pos[1] -= 1
            elif direct == 1:
        ...
        for i in range(len(self.agents)):
            self.agents[i].set_goal(self.prey.pos)
    ...
    def step(self, actions):
        ...
        return observations, rewards, dones, flag_track
